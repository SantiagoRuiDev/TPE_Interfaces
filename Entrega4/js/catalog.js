...
const renderCatalogPage = (catalog, page) => {
  ...
  const list =
    selectedCategory == "" ? catalog.items[page] : catalog.filtered[page];
...
const toggleNavigation = (catalog) => {
  catalog.nextBtn.addEventListener("click", () => {
    ...
    if (selectedCategory == "") {
      ...
      nextPage =
        catalog.currentPage + 1 < catalog.filtered.length
          ? catalog.currentPage + 1
          : 0;
...
const listenCategorySelector = (catalog) => {
  ...
  category_reset.addEventListener("click", () => {
    // Si se clickea el boton de "Todo"
...
const filterCatalogByCategory = (catalog) => {
  ...
  for (const page of catalog.items) {
    // Recorremos las paginas tal cual las obtenemos de la API
    for (const game of page) {
      if (game.category == selectedCategory) {
        // Chequeamos si la categoria es la misma
  ...
  if (filteredPage.length > 0 && filteredPage.length < items_per_page) {
    // Si la pagina no quedo completa (menos de 4 items)
    pagesManager.push(filteredPage);
  }
...
const enableCartListing = (catalog) => {
  const buy_action_button = document.querySelectorAll(
    "." + catalog.name + "-action-button"
  ...
  buy_action_button.forEach((btn, index) => {
    btn.addEventListener("click", () => {
      ...
      if (!cart.find((p) => p.id == product.id)) {
        handleProductButton(btn, "AddedToCart.svg", "Agregado");
        addProductToCart(product);
      } else {
        handleProductButton(btn, "Cart_Icon.svg", "Comprar");
...
// Funcion para cambiar el boton de la card (De comprar a agregado o viceversa)
const handleProductButton = (btn, url, text) => {
  btn.textContent = ""; // Limpia el contenido
...
const renderCartProducts = () => {
  ...
  cart.forEach((product) => {
    if (product.discount)
      cart_overview.innerHTML += mapProductDiscountCartCard(product);
...
const renderCartPricing = () => {
  ...
  if (cart.length > 0) {
    ...
    for (const p of cart) {
      if (p.discount)
        cart_total += p.price - (p.price * p.discount_percentage) / 100;
      else cart_total += p.price;
    }
    price_text.innerHTML = "Total a pagar: $" + cart_total.toFixed(2);
